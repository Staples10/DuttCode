...
class ConfocalPoint(Experiment):
    ...
    _DEFAULT_SETTINGS = [
        ...
        Parameter('graph_params',
                  [Parameter('refresh_rate',0.01,float,'For continuous counting this is the refresh rate of the graph in seconds (= 1/frames per second)'),
                   ...
                   Parameter('font_size',14,int,'font size to make it easier to see on the fly if needed')
                   ])
    ]
    ...
    def _function(self):
        ...
        if self.settings['continuous'] == False:
            sleep((self.settings['count_time']*1.2)/1000)#sleep for 1.2 times the count time to ensure enough time for counts. note this does not affect actually counting window
            ...
            while self._abort == False:     #self._abort is defined in experiment.py and is true false while running and set false when stop button is hit
                ...
                self.progress = j   #this is a infinite loop till stop button is hit; progress & updateProgress is only here to update plot
    ...
    def _plot(self, axes_list, data=None):
        '''
        This function plots the data. It is triggered when the updateProgress signal is emited and after the _function is executed.
        '''
        if data is None:
            data = self.data
        ...
        if data is not None and data is not {}:
            ...
            axes_list[1].set_xlim(left=0,right=(self.settings['graph_params']['length_data']+100))
